...
export class ProvenanceTreeVisualization {
  ...
  constructor(
    ...
    this.svg = d3
      .select<d3.BaseType, ExtendedProvenanceNode>('body')
      .append('svg')
    ...
    this.tree = customtree<ExtendedProvenanceNode>()
      ...
      .nodeSize([20, 20]);
    this.root = d3.hierarchy<ExtendedProvenanceNode>(
      this.rootNode,
      (d: ExtendedProvenanceNode) => {
        return d.newchildren;
      },
    );

    const treeData: d3.HierarchyPointNode<ExtendedProvenanceNode> = this.tree(
      this.root,
    );
    this.update(treeData);
  }

  public static nodeSize(node: ProvenanceNode): number {
    return 1;
  }

  public static nodePosition(
    node: d3.HierarchyPointNode<ExtendedProvenanceNode>,
  ): IPosition {
    return {
      x: 0,
      y: 0,
  ...
  public convertToExtendedNode(
    ...
    const result: ExtendedProvenanceNode = {
      ...
      actionResult: isStateNode(provenanceNode)
        ? provenanceNode.actionResult
        : null,
  ...
  public update(rootNode: d3.HierarchyPointNode<ExtendedProvenanceNode>) {
    ...
    const nodes: Array<
      d3.HierarchyPointNode<ExtendedProvenanceNode>
    > = treeData.descendants();
    const links: Array<
      d3.HierarchyPointLink<ExtendedProvenanceNode>
    > = treeData.links();
    ...
    // Update the nodes...
    const allExistingNodes = this.svg
      .selectAll<d3.BaseType, ExtendedProvenanceNode>('g.node')
    ...
    const allNewNodes = allExistingNodes
      .enter()
      .append('g')
      .classed('node', true)
      .classed('selected', (d) => {
    ...
    allNewNodes.append('circle').attr('r', 1);
    ...
    allNewNodes
      ...
      .text(function(d: d3.HierarchyPointNode<ExtendedProvenanceNode>) {
        return d.data.label;
      });

    // UPDATE
    const allNodes = allNewNodes.merge(allExistingNodes);
    ...
    allNodes
      ...
      .attr(
        'transform',
        function(d: d3.HierarchyPointNode<ExtendedProvenanceNode>) {
    ...
    const toBeRemovedNodes = allExistingNodes
      .exit()
      .transition()
      .duration(this.duration)
      .attr('transform', function(
    ...
    toBeRemovedNodes.select('circle').attr('r', 1e-6);

    // On exit reduce the opacity of text labels
    toBeRemovedNodes.select('text').style('fill-opacity', 1e-6);
    ...
    function diagonal(
      s: d3.HierarchyPointNode<ExtendedProvenanceNode>,
      t: d3.HierarchyPointNode<ExtendedProvenanceNode>,
    ) {
      const path =
        'M' +
        s.x +
