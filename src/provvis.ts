...
export class StateRepresentation {
  ...
  setParent(lookup: { [id: number]: StateRepresentation }, line: StateRepresentation[]) {
    const p = this.s.previousState;
    if (p) {
      this.parent = lookup[p.id];
      if (line.indexOf(this) >= 0) {
        //ensure the element is the first of its parent's children
  ...
  get intent():Intent {
    if (this.s.name.includes('Rotation') || this.s.name.includes('Zoom') || this.s.name.includes('Translation')) {
      return Intent.Explore;
    } else if (this.s.name.includes('Select')) {
      return Intent.Select;
    } else if (this.s.name.includes('Statistics?')) {
      return Intent.Derive;
    } else if (this.s.name.includes('Configure') || this.s.name.includes('SliceHandle') || this.s.name.includes('Visibility')) {
      return Intent.Configure;
    } else if (this.s.name.includes('Annotate')) {
      return Intent.Annotate;
    } else if (this.s.name.includes('Provenance')) {
      return Intent.Provenance;
    }
    return Intent.Unknown;
  }

  static isKeynode(d: StateRepresentation): boolean {
    return (d.parent === null || d.children.length === 0 || d.children.length > 1 || (d.children.length === 1 && d.children[0].intent !== d.intent));
  }
  ...
  static toStateRepresentation(graph: provenance.ProvenanceGraph, highlight: any, options: any = {}): StateRepresentation[] {
    ...
    const unfilteredStates = graph.states;
    const filteredStates = unfilteredStates.filter((state) => {
      return isStateNodeKeynode(state);
    });

    const size = unfilteredStates.length;

    const toStateRepresentation = (s) => {
      ...
      //Set the selected item highlight.
      r.selected = s === selected;

      r.isKeynode = isStateNodeKeynode(s);

      //Count the number of previous nodes that were not keynodes
      r.keynodeSize = 1;

      if (r.isKeynode) {
        let parent = s.previousState;
        while (parent && !isStateNodeKeynode(parent)) {
          r.keynodeSize++;
    ...
    const stateRepresentations = (lod < LevelOfDetail.Medium ? selectedPath : unfilteredStates).map(toStateRepresentation);
    // const filteredStateRepresentations = (lod < LevelOfDetail.Medium ? selectedPath : filteredStates).map(toStateRepresentation);

    //route to path = 1
    const line = selected.path.map((p) => lookup[p.id]);
    //build tree
    stateRepresentations.forEach((s) => s.setParent(lookup, line));

    this.layout(stateRepresentations, line);

    stateRepresentations.forEach((s) => s.setMyKeynode());

    //boost all on the right side if they are small to medium

    return stateRepresentations;
  }
  ...
  private static layout(states: StateRepresentation[], line: StateRepresentation[]) {
    ...
    let outerArrayLength = byLevel.length;

    while (byLevel[outerArrayLength - 1].length > 0) {
      byLevel.push(
        [].concat.apply(
          [], byLevel[outerArrayLength - 1].map(
            (c) => c.children.slice()
          )
        )
      );
  ...
  static render($elem: d3.Selection<StateRepresentation>, layoutedProvvis: LayoutedProvVis) {
    const groupedDisplay = layoutedProvvis.option('groupedDisplay');
    console.log('groupedDisplay : ' + groupedDisplay);
    $elem
      ...
      .classed('keynode', (d) => (groupedDisplay && d.isKeynode))
      .classed('intent-explore', (d) => (groupedDisplay && d.intent === Intent.Explore))
      .classed('intent-select', (d) => (groupedDisplay && d.intent === Intent.Select))
      .classed('intent-derive', (d) => (groupedDisplay && d.intent === Intent.Derive))
      .classed('intent-configure', (d) => (groupedDisplay && d.intent === Intent.Configure))
      .classed('intent-annotate', (d) => (groupedDisplay && d.intent === Intent.Annotate))
      .classed('intent-provenance', (d) => (groupedDisplay && d.intent === Intent.Provenance))
      .classed('hidden', (d) => (groupedDisplay && !d.shouldShow()))
      .attr('data-doi', (d) => d.doi)
      .attr('title', (d) => d.name);

    // $elem.select('i.glyph').transition().style({
    //   'font-size': (d) =>  4 + d.keynodeSize * 4 + 'px'
    // });
...
export class LayoutedProvVis extends vis.AVisInstance implements vis.IVisInstance {
  ...
  // Toggle functionality for new grouped display function
  public toggleGrouping() {
    this.option('groupedDisplay', !this.options.groupedDisplay);
    this.update();
    // this.options.groupedDisplay = !this.options.groupedDisplay;
  }
    ...
    $provenanceGraph.html(`
      ...
      <div>
        <h2>
          ...
          <a href="#" class="btn-group"><i class="fa fa-object-group"></i></a>
      ...
      <div class="intent-legend">
        <div class="btn-group-vertical" data-toggle="buttons">
          <label class="btn btn-default btn-xs intent-explore" title="intent explore">
            <input type="checkbox" name="intent" value="explore"> <i class="fa fa-circle glyph"></i> Explore
          </label>
          <label class="btn btn-default btn-xs intent-select" title="intent select">
            <input type="checkbox" name="intent" value="select"> <i class="fa fa-circle glyph"></i> Select
          </label>
          <label class="btn btn-default btn-xs intent-derive" title="intent derive">
            <input type="checkbox" name="intent" value="derive"> <i class="fa fa-circle glyph"></i> Derive
          </label>
          <label class="btn btn-default btn-xs intent-configure" title="intent configure">
            <input type="checkbox" name="intent" value="configure"> <i class="fa fa-circle glyph"></i> Configure
          </label>
          <label class="btn btn-default btn-xs intent-provenance" title="intent provenance">
            <input type="checkbox" name="intent" value="provenance"> <i class="fa fa-circle glyph"></i> Provenance
          </label>
          <label class="btn btn-default btn-xs keynode" title="intent keynode">
            <input type="checkbox" name="intent" value="keynode"> <i class="fa fa-circle glyph"></i> Keynode
          </label>
        </div>
      </div>
      <!--
  ...
  update() {
    ...
    $states.call(StateRepresentation.render, this);
